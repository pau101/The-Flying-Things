...
public class ModelEnchantedBroom extends SegmentedModel<EntityEnchantedBroom> {
    ...
    @Override
    public void setupAnim(EntityEnchantedBroom entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
    }
    ...
    public void renderToBuffer(MatrixStack matrixStack, IVertexBuilder iVertexBuilder, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
        ...
        if (ConfigFlyingThings.client.renderEffect.get()) {
            ...
            parts().forEach(part -> part.render(matrixStack, ivertexbuilder2, packedLight, packedOverlay, red, green, blue, alpha));
        }
    }
